...
export default function UploadImage({ addedPhoto, onChange }) {
    const [photolinks, setPhotolinks] = useState('');
    async function addPhotoByLink(ev) {
        ev.preventDefault();
        const { data } = await axios.post('/upload-by-link', { link: photolinks })
        console.log(data);
            ...
            <div className=" mt-3 grid gap-2 grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
                {addedPhoto.length > 0 && addedPhoto.map(link => (
                    <div key={link} className="h-32 flex relative">
                        <img className="rounded-2xl w-full object-cover " src={config.production.backendUrl + "/controller/images/" + link} alt="" srcSet="" />
                        <button onClick={ev => removePhoto(ev,link)} className="absolute bottom-1 p-2 right-1 bg-white rounded-full bg-opacity-70 cursor-pointer ">
                        ...
                        <button onClick={ev => selectAsMain(ev,link)} className="absolute top-2 left-2 bg-black rounded-full bg-opacity-30 cursor-pointer p-2 ">
                            {link ===addedPhoto[0] && (
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#FF55BB" className="w-4 h-4">
                                <path fillRule="evenodd" d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z" clipRule="evenodd" />
                              </svg>
                              
                            )}
                            {link !== addedPhoto[0] && (
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="#FF55BB" className="w-4 h-4">
                ...
                <label className=" flex justify-center items-center bg-transparent border rounded-2xl p-2 text-2xl text-gray-600">
