...
const ToggglBar = ({ switchAddons }) => {
  ...
  return (
    <div className="flex justify-around items-center border max-w-lg border-gray-400 w-5/6 md:w-2/3 px-2 md:px-5 mx-auto gap-4 rounded-xl py-1 md:py-4">
      ...
      <div onClick={handleClick} className="w-10 md:w-16 py-[1px] px-[1px] h-4 md:h-7 flex items-center rounded-3xl   border-[1px] border-gray-400 relative ">
        <div className={`bg-primary w-4  h-4 md:w-6 md:h-6 rounded-full absolute  ${price ? 'translate-x-[100%] md:translate-x-[150%]' : 'translate-x-0'} transition-all duration-700 ease-in-out `}></div>
      </div>
    </div>
  )
}
